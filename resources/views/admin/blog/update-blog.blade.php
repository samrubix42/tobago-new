...
<div class="max-w-4xl mx-auto p-6 bg-white rounded-2xl border border-slate-100">
    ...
    <div class="space-y-4">
        ...
        <div>
            ...
            <div
                wire:ignore
                wire:key="blog-update-editor-{{ $id }}"
                x-data="{
                    editor: null,
                    init() {
                        this.$nextTick(() => setTimeout(() => this.start(), 50));
                    },
                    start() {
                        const textarea = this.$refs.textarea;
                        if (!textarea || !textarea.isConnected || !window.tinymce) {
            ...
            >
                <textarea
                    id="blog-update-content-{{ $id }}"
                    x-ref="textarea"
                    class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-gray-900">{{ $content }}</textarea>
            </div>
