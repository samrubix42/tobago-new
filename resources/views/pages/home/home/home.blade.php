<div>
    ...
    <section id="collection" class="max-w-7xl mx-auto px-4 sm:px-6 py-20">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
            ...
            @endphp
            <div class="group rounded-2xl border border-subtle bg-[#0b0d0f] p-3 sm:p-4 transition hover:-translate-y-1 hover:border-cyan-300/30 hover:shadow-2xl hover:shadow-cyan-900/30 block relative {{ ($product->is_out_of_stock || $product->stock <= 0) ? 'opacity-70 grayscale-[0.5]' : '' }}">
...
<div class="mt-8 flex justify-center">
    <a href="{{ route('products.category.subcategory', ['category' => 'premium-hookah', 'subcategory' => 'tobac-go-hookah']) }}" wire:navigate class="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-full border border-white/20 bg-white/[0.04] px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/85 transition hover:border-white/35 hover:bg-white/[0.08] hover:text-white">
...
<section id="new-arrivals" class="relative max-w-7xl mx-auto px-4 sm:px-6 py-20 overflow-hidden">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        ...
        ?? optional($product->images->first())->image;
        @endphp
        <div class="group rounded-2xl border border-subtle bg-[#0b0d0f] p-3 sm:p-4 transition hover:-translate-y-1 hover:border-purple-300/30 hover:shadow-2xl hover:shadow-purple-900/30 block relative {{ ($product->is_out_of_stock || $product->stock <= 0) ? 'opacity-70 grayscale-[0.5]' : '' }}">
            <a href="{{ route('product', $product->slug) }}" wire:navigate class="absolute inset-0 z-10 rounded-2xl" aria-label="{{ $product->name }}"></a>
            @if($product->is_out_of_stock || $product->stock <= 0)
                <div class="absolute top-5 left-5 z-20">
                <span class="px-2 py-1 rounded-lg bg-red-500/20 border border-red-500/30 text-[9px] font-bold text-red-400 uppercase tracking-widest backdrop-blur-md">
    ...
    <div class="space-y-10">
        ...
        <div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
                ...
                ?? optional($product->images->first())->image;
                @endphp
                <div class="group rounded-2xl border border-subtle bg-[#0b0d0f] p-3 sm:p-4 transition duration-300 hover:-translate-y-1 hover:border-white/20 hover:shadow-2xl hover:shadow-black/30 block relative {{ ($product->is_out_of_stock || $product->stock <= 0) ? 'opacity-70 grayscale-[0.5]' : '' }}">
                    <a href="{{ route('product', $product->slug) }}" wire:navigate class="absolute inset-0 z-10 rounded-2xl" aria-label="{{ $product->name }}"></a>
                    @if($product->is_out_of_stock || $product->stock <= 0)
                        <div class="absolute top-5 left-5 z-20">
                        <span class="px-2 py-1 rounded-lg bg-red-500/20 border border-red-500/30 text-[9px] font-bold text-red-400 uppercase tracking-widest backdrop-blur-md">
...
<script>
    function testimonialSlider() {
        return {
            ...
            init() {
                const dataNode = document.getElementById('testimonial-items-data');
                this.items = dataNode ? JSON.parse(dataNode.textContent || '[]') : [];
                this.update();
